...
public class TransferInfServiceImpl implements TransferInfService {
	...
	public JSONObject queryAllInf(TransferInf t) {
		...
		try {
			...
			return JsonFactory.getJSONObject(Configure.CONTENTTRUECODE, t1,t4);
	...
	public JSONObject querySecondInf(TransferInf t) {
		...
		try {
			List<Ticket> t1=ticketService.getTicketInf(q1);
			if(t1.size()==0)
				return JsonFactory.getJSONObject("当天无可中转的车次", Configure.DBFALSECODE);
			...
			return JsonFactory.getJSONObject(Configure.CONTENTTRUECODE,t4);
		} catch (ContentException e) {
			return JsonFactory.getJSONObject("请求内容有误", Configure.CONTENTFAULTCODE);
		} catch (Exception e) {
			return JsonFactory.getJSONObject("数据库发生错误，请及时反馈给我们", Configure.DBFALSECODE);
